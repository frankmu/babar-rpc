...
@Component
public class BabarRPCServer implements DisposableBean{
	...
	@Autowired
	private BabarRPCServiceRegistry babarServiceRegistry;
	...
	public void handleContextRefresh(ContextRefreshedEvent event){
		...
		EventExecutorGroup executorGroup = new DefaultEventExecutorGroup(babarServerWorkerThreadNumber, new BabarRPCServerWorkerThreadFactory());
		try {
			ServerBootstrap b = new ServerBootstrap();
			b.group(bossGroup, workerGroup);
			b.channel(NioServerSocketChannel.class);
			b.childHandler(new BabarRPCServerInitializer(handlerMap, executorGroup));
	...
	private void registerService(ContextRefreshedEvent event){
		...
		Map<String, Object> serviceBeanMap = ctx.getBeansWithAnnotation(BabarRPCService.class);
		for(Object bean : serviceBeanMap.values()){
			String interfaceName = bean.getClass().getAnnotation(BabarRPCService.class).value().getName();
